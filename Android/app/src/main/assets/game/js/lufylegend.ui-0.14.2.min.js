var LButtonSample1 = (function() {
	...
	LButtonSample1.prototype.clone = function() {
		var s = this, name = s.labelText.text, size = s.labelText.size, font = s.labelText.font, color = s.labelText.color, a = new LButtonSample1(name, size, font, color);
...
var LButtonSample2 = (function() {
	...
	LButtonSample2.prototype.clone = function() {
		var s = this, name = s.labelText.text, size = s.labelText.size, font = s.labelText.font, color = s.labelText.color, a = new LButtonSample2(name, size, font, color);
		a.backgroundColor = s.backgroundColor;
		a.x = s.x;
		a.y = s.y;
		return a;
...
var LMenubar = (function() {
	...
	LMenubar.prototype.openMainMenu = function(index) {
		var self = this;
		self.mousedown({
			clickTarget : self.getChildAt(index + 1)
		});
	};
...
var LRange = (function() {
	function LRange() {
		...
		if (arguments.length == 0) {
			...
			param = {
				backLayer : arguments[0],
				selectLayer : arguments[1]
			};
		} else {
...
var LScrollbar = (function() {
	...
	LScrollbar.prototype.dragRangeUpdate = function(w, h) {
		...
		s._showObject.dragRange = new LRectangle(!s._wVisible || w < s._maskW ? 0 : s._maskW - w, !s._hVisible || h < s._maskH ? 0 : s._maskH - h, s._wVisible && w > s._maskW ? w - s._maskW : 0, s._hVisible && h > s._maskH ? h - s._maskH : 0);
	};
...
var LWindow = (function() {
	...
	LWindow.prototype.clone = function() {
		var s = this, a = new LWindow({
			width : s.style.width,
			height : s.style.height,
			title : s.style.title,
			header : s.style.header.clone(),
			closeButton : s.style.closeButton.clone(),
			background : s.style.background.clone()
		});
...
LTable.prototype.setCell = function(child, row, col) {
	var s = this;
	if (!child || !s.childList[row] || !s.childList[row].childList[col])
		return -1;
...
var LListView = (function() {
	...
	LListView.DragEffects = {
		None : "none",
		Momentum : "momentum",
		MomentumAndSpring : "momentumAndSpring"
	};
	LListView.Direction = {
	...
	LListView.prototype.clickOnChild = function(selfX, selfY, type) {
		...
		if (index < self._ll_items.length) {
			...
			var event = {
				...
				offsetX : mouseX,
				offsetY : mouseY,
				selfX : (x % self.cellWidth),
				selfY : (y % self.cellHeight)
			};
			if (type == "touch") {
				child.onTouch(event);
			} else {
