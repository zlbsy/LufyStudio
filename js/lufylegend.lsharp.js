...
ScriptImg.addImg = function(value, start, end) {
	...
	if (wNum)
		bitmap.scaleX = wNum / bitdata.width;
	//如果设置了高，则通过scaleX计算LBitmap的高
	if (hNum)
		bitmap.scaleY = hNum / bitdata.height;
	//设置透明度
...
ScriptIF.getIF = function(value) {
	...
	while (startifIndex < script.lineList.length) {
		...
		if (script.lineList[startifIndex].indexOf("elseif") >= 0) {
			...
			eCount = 0;
			while (sCount > eCount) {
				if (script.lineList[startifIndex].indexOf("if") >= 0 && script.lineList[startifIndex].indexOf("else") < 0 && script.lineList[startifIndex].indexOf("end") < 0) {
					sCount++;
				} else if (script.lineList[startifIndex].indexOf("endif") >= 0) {
					eCount++;
				}
...
ScriptButton.addButton = function(value, start, end) {
	...
	//按钮名称
	var labelStr = lArr[2];
	//按钮上的文字，如果设置为null，则不显示文字
	var x = parseInt(lArr[3]);
	//按钮坐标
	var y = parseInt(lArr[4]);
	//按钮坐标
	var dataUp = lArr[5];
	//按钮弹起样式的bitmapData对象名称
	var dataOver = lArr[6];
	//按钮点击后样式的bitmapData对象名称
...
LRPGCharacter.analysis = function(value) {
	...
	var end = value.indexOf(")");
	switch(value.substr(0,start)) {
		case "RPGCharacter.move":
			//进行相对坐标移动
			LRPGCharacter.move(value, start, end);
			break;
		case "RPGCharacter.moveTo":
			//进行绝对坐标移动
			LRPGCharacter.moveTo(value, start, end);
			break;
		case "RPGCharacter.moveToCharacter":
			//以某人物为参考物进行相对移动坐标
...
LSGJSingleCombatScript = function() {
};
LSGJSingleCombatScript.analysis = function(value) {
	var start = value.indexOf("(");
	var end = value.indexOf(")");
	switch(value.substr(0,start)) {
		case "SGJSingleCombat.dodge":
			LSGJSingleCombatScript.dodge(value, start, end);
			break;
		case "SGJSingleCombat.moveTo":
			LSGJSingleCombatScript.moveTo(value, start, end);
			break;
...
LSGJSingleCombatScript.dodge = function(value, start, end) {
	...
	if(character.isLeft){
		character.setCoordinate(character.x - BattleCharacterSize.width, character.y);
	}else{
		character.setCoordinate(character.x + BattleCharacterSize.width, character.y);
	}
	LGlobal.script.analysis();
...
LSGJSingleCombatScript.moveTo = function(value, start, end) {
	//params:charaId,absolute,coordinate
	//params:charaId,relative,direction(forward,backwards),distance
	var params = value.substring(start + 1, end).split(",");
	var character = LMvc.SingleCombatController.view.characterLayer.childList.find(function(child){
		return child.constructor.name == "SingleCombatCharacterView" && child.data.id() == parseInt(params[0]);
	});
	...
	if(params[1] == "absolute"){
		...
		LSGJSingleCombatScript.moveToRelative(character,params[2],parseInt(params[3]));
	}
};
...
LSGJSingleCombatScript.moveToRelative = function(character,direction,distance) {
	if(character.isLeft){
		character.moveTo(character.x + (direction == "forward" ? distance : -distance), character.y);
	}else{
		character.moveTo(character.x + (direction == "forward" ? -distance : distance), character.y);
	}
};
